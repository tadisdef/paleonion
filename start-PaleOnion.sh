...
if [ -L "$myname" ]; then
	...
	if [ "$?" -eq 0 ]; then
		...
		if [ "$?" -ne 0 ]; then
			# Ugh.
			complain "start-PaleOnion cannot be run using a symlink on this operating system."
		fi
	fi
fi
...
printf "\nLaunching PaleOnion for Linux in ${HOME}\n"
cd "${HOME}"
# XXX Someday we should pass whatever command-line arguments we got
# (probably filenames or URLs) to Firefox.
./Browser/palemoon -no-remote -profile Data/Browser/profile.default
exitcode="$?"
if [ "$exitcode" -ne 0 ]; then
	complain "PaleOnion exited abnormally.  Exit code: $exitcode"
	exit "$exitcode"
else
	printf '\nPaleOnion exited cleanly.\n'
fi
